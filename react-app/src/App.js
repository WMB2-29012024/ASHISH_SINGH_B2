// import Task from './tasks/task1/Task';
// import Task2 from './tasks/task2/task';
// import Task3 from './tasks/task3/task';
// import Task5 from './tasks/task5/Task';
// import Task6 from './tasks/task6/task';
// import Task7 from './tasks/task7/task';
// import Task8 from './tasks/task8/task';
// import Task9 from './tasks/task9/task';
// import Task10 from'./tasks/task10/task';
// import Task11 from'./tasks/task11/task';
// import Task12 from'./tasks/task12/task';
// import Task13 from'./tasks/task13/task';
// import Task14 from './tasks/task14/task';
import Task15 from "./tasks/task15/Task";

function App() {
  return (
    <div>
     
      <Task15/>
    </div>
  );
}

export default App;
